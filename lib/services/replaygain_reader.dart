...
class ReplayGainData {
  ...
  bool get hasAnyData => trackGain != null || albumGain != null || trackPeak != null || albumPeak != null;
...
class ReplayGainReader {
  ...
  static Future<ReplayGainData> readFromUrl(String url) async {
    try {
      ...
      if (bytes.isNotEmpty) {
        debugPrint('Received ${bytes.length} bytes, first 16 bytes: ${bytes.take(16).map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');
  ...
  static ReplayGainData _parseReplayGainFromBytes(Uint8List bytes) {
    ...
    if (bruteForceData.hasAnyData) {
      debugPrint('Found ReplayGain data via brute force search: $bruteForceData');
  ...
  static ReplayGainData _parseID3v2Tags(Uint8List bytes) {
    ...
    debugPrint('Found ID3v2.$version.$revision tag, size: $tagSize, available: ${bytes.length}');
    ...
    while (offset + 10 < tagSize) {
      ...
      if (version >= 4) {
        ...
        frameSize = (bytes[offset] << 21) | (bytes[offset + 1] << 14) | 
      ...
      if (frameId == 'TXXX') {
        ...
        if (replayGainData.trackGain != null) trackGain = replayGainData.trackGain;
        if (replayGainData.albumGain != null) albumGain = replayGainData.albumGain;
        if (replayGainData.trackPeak != null) trackPeak = replayGainData.trackPeak;
        if (replayGainData.albumPeak != null) albumPeak = replayGainData.albumPeak;
      }
  ...
  static ReplayGainData _parseReplayGainTXXXFrame(Uint8List frameData) {
    ...
    if (encoding == 1) {
      ...
      int descEnd = offset;
      while (descEnd + 1 < frameData.length && (frameData[descEnd] != 0 || frameData[descEnd + 1] != 0)) {
        descEnd += 2;
      }
      ...
      if (descEnd - offset >= 2 && frameData[offset] == 0xFF && frameData[offset + 1] == 0xFE) {
        descStart += 2; // Skip BOM
      }
      
      try {
        description = utf8.decode(frameData.sublist(descStart, descEnd), allowMalformed: true).toUpperCase();
      } catch (e) {
        description = String.fromCharCodes(frameData.sublist(descStart, descEnd)).toUpperCase();
      }
      ...
      try {
        value = utf8.decode(frameData.sublist(offset), allowMalformed: true).trim();
      } catch (e) {
      ...
      description = String.fromCharCodes(frameData.sublist(offset, descEnd)).toUpperCase();
      offset = descEnd + 1;
    ...
    final cleanDescription = description.replaceAll(RegExp(r'[^\x20-\x7E]'), '?');
    ...
    double? parseGain(String gainStr) {
      ...
      final cleanStr = gainStr.replaceAll(RegExp(r'\s*db\s*$', caseSensitive: false), '');
  ...
  static ReplayGainData _parseAPETagItems(Uint8List bytes, int tagOffset) {
    ...
    final trackGainMatch = RegExp(r'REPLAYGAIN_TRACK_GAIN[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (trackGainMatch != null) {
      trackGain = double.tryParse(trackGainMatch.group(1)!);
    }
    
    final albumGainMatch = RegExp(r'REPLAYGAIN_ALBUM_GAIN[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (albumGainMatch != null) {
      albumGain = double.tryParse(albumGainMatch.group(1)!);
    }
    
    final trackPeakMatch = RegExp(r'REPLAYGAIN_TRACK_PEAK[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (trackPeakMatch != null) {
      trackPeak = double.tryParse(trackPeakMatch.group(1)!);
    }
    
    final albumPeakMatch = RegExp(r'REPLAYGAIN_ALBUM_PEAK[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
  ...
  static ReplayGainData _parseVorbisComments(Uint8List bytes) {
    ...
    final trackGainMatch = RegExp(r'REPLAYGAIN_TRACK_GAIN=(-?\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (trackGainMatch != null) {
      trackGain = double.tryParse(trackGainMatch.group(1)!);
    }
    
    final albumGainMatch = RegExp(r'REPLAYGAIN_ALBUM_GAIN=(-?\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (albumGainMatch != null) {
      albumGain = double.tryParse(albumGainMatch.group(1)!);
    }
    
    final trackPeakMatch = RegExp(r'REPLAYGAIN_TRACK_PEAK=(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (trackPeakMatch != null) {
      trackPeak = double.tryParse(trackPeakMatch.group(1)!);
    }
    
    final albumPeakMatch = RegExp(r'REPLAYGAIN_ALBUM_PEAK=(\d+\.?\d*)', caseSensitive: false).firstMatch(text);
    if (albumPeakMatch != null) {
      albumPeak = double.tryParse(albumPeakMatch.group(1)!);
    }
  ...
  static ReplayGainData _parseMP4Tags(Uint8List bytes) {
    ...
    while (offset + 8 < bytes.length) {
      ...
      final atomSize = (bytes[offset] << 24) | (bytes[offset + 1] << 16) | 
                      (bytes[offset + 2] << 8) | bytes[offset + 3];
    ...
    final trackGainMatch = RegExp(r'----:com\.apple\.iTunes:replaygain_track_gain[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    if (trackGainMatch != null) {
      trackGain = double.tryParse(trackGainMatch.group(1)!);
      debugPrint('Found iTunes ReplayGain track gain: $trackGain');
    }
    
    final albumGainMatch = RegExp(r'----:com\.apple\.iTunes:replaygain_album_gain[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    if (albumGainMatch != null) {
      albumGain = double.tryParse(albumGainMatch.group(1)!);
      debugPrint('Found iTunes ReplayGain album gain: $albumGain');
    }
    
    final trackPeakMatch = RegExp(r'----:com\.apple\.iTunes:replaygain_track_peak[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    if (trackPeakMatch != null) {
      trackPeak = double.tryParse(trackPeakMatch.group(1)!);
      debugPrint('Found iTunes ReplayGain track peak: $trackPeak');
    }
    
    final albumPeakMatch = RegExp(r'----:com\.apple\.iTunes:replaygain_album_peak[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    if (albumPeakMatch != null) {
      albumPeak = double.tryParse(albumPeakMatch.group(1)!);
    ...
    if (trackGain == null) {
      final stdTrackGainMatch = RegExp(r'replaygain_track_gain[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    ...
    if (albumGain == null) {
      final stdAlbumGainMatch = RegExp(r'replaygain_album_gain[^\d-]*(-?\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    ...
    if (trackPeak == null) {
      final stdTrackPeakMatch = RegExp(r'replaygain_track_peak[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
    ...
    if (albumPeak == null) {
      final stdAlbumPeakMatch = RegExp(r'replaygain_album_peak[^\d]*(\d+\.?\d*)', caseSensitive: false).firstMatch(textData);
  ...
  static ReplayGainData _parseItunesMetadata(Uint8List bytes) {
    ...
    final textData = String.fromCharCodes(bytes);
    debugPrint('iTunes metadata text sample: ${textData.substring(0, math.min(200, textData.length))}');
    ...
    for (final pattern in patterns) {
      final regex = RegExp('$pattern[^\\d-]*(-?\\d+\\.?\\d*)', caseSensitive: false);
  ...
  static ReplayGainData _bruteForceSearch(Uint8List bytes) {
    try {
      ...
      try {
        ...
        final filteredBytes = bytes.where((b) => b >= 32 && b <= 126 || b == 10 || b == 13).toList();
    ...
    final patterns = [
      ...
      RegExp(r'replaygain_track_gain[^-\d]*(-?\d+\.?\d*)', caseSensitive: false),
      RegExp(r'replaygain_album_gain[^-\d]*(-?\d+\.?\d*)', caseSensitive: false),
      RegExp(r'replaygain_track_peak[^\d]*(\d+\.?\d*)', caseSensitive: false),
      RegExp(r'replaygain_album_peak[^\d]*(\d+\.?\d*)', caseSensitive: false),
      
      // iTunes format
      RegExp(r'----:com\.apple\.iTunes:replaygain_track_gain[^-\d]*(-?\d+\.?\d*)', caseSensitive: false),
      RegExp(r'----:com\.apple\.iTunes:replaygain_album_gain[^-\d]*(-?\d+\.?\d*)', caseSensitive: false),
      RegExp(r'----:com\.apple\.iTunes:replaygain_track_peak[^\d]*(\d+\.?\d*)', caseSensitive: false),
      RegExp(r'----:com\.apple\.iTunes:replaygain_album_peak[^\d]*(\d+\.?\d*)', caseSensitive: false),
    ...
    for (final pattern in patterns) {
      ...
      if (match != null) {
        ...
        if (patternStr.contains('track_gain') || patternStr.contains('track_gain')) {
    ...
    for (final pattern in dbPatterns) {
      ...
      for (final match in matches) {
        ...
        if (value != null && value >= -30 && value <= 15) {
          ...
          debugPrint('Found potential ReplayGain value: ${match.group(0)} -> $value');
